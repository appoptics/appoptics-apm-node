...
Span.makeEntrySpan = function makeEntrySpan (name, settings, kvpairs) {
  ...
  // if not sampling make a single skeleton span that will be used for all other spans.
  span.skeleton = undefined;
  if (!this.doSample) {
    span.skeleton = new Span('skeleton', {traceTaskId: span.events.entry.event});
    // the skeleton hasn't been used yet, so decrement the created and active event counts as
    // if they were not created.
    Event.incrementStats(-2);
    span.skeleton.isSkeleton = true;
  }
